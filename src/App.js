...
function BulletTextarea({value,onChange,placeholder,style:s}){
  ...
  const hk=e=>{if(e.key==="Enter"){e.preventDefault();const v=e.target.value;const p=e.target.selectionStart;const n=v.slice(0,p)+"\n\u2022 "+v.slice(p);onChange({target:{value:n}});setTimeout(()=>{e.target.selectionStart=e.target.selectionEnd=p+3;if(ref.current){ref.current.scrollTop=ref.current.scrollHeight;}},0);}};
  const hc=e=>{let v=e.target.value;
    // Auto-add first bullet
    if(v&&!v.startsWith("\u2022")&&v.trim().length>0&&!value)v="\u2022 "+v;
    // Auto capitalize first letter after each bullet point
    v=v.replace(/\u2022 ([a-z])/g,(m,c)=>"\u2022 "+c.toUpperCase());
    onChange({target:{value:v}});
  };
  return<textarea ref={ref} value={value} onChange={hc} onKeyDown={hk} placeholder={placeholder} rows={4} style={{...s,resize:"vertical",minHeight:"100px",overflowY:"auto"}}/>;
}
...
export default function App(){
  ...
  // Active Jobs helpers
  const saveActiveJob=()=>{
    if(!newJobName.trim()){showToast("Job name required");return;}
    const now=new Date().toISOString();
    const jobs=[...(activeJobs||[])];
    const d={name:newJobName.trim().toUpperCase(),customer:newJobCustomer.trim(),address:newJobAddress.trim(),lastModified:now};
    if(editingActiveJob!==null){jobs[editingActiveJob]=d;}
    else{jobs.push(d);}
    saveToFB("activeJobs",jobs);setNewJobName("");setNewJobAddress("");setNewJobCustomer("");setEditingActiveJob(null);setShowAddJob(false);showToast(editingActiveJob!==null?"Updated":"Job added");
  };
  ...
  if(showAddJob)return(
    <div style={{minHeight:"100vh",background:t.bg,fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <Header title={editingActiveJob!==null?"Edit Job":"Add New Job"} onBack={()=>{setShowAddJob(false);setEditingActiveJob(null);setNewJobName("");setNewJobAddress("");setNewJobCustomer("");}} onHome={goHome}/>
      <div style={{padding:"20px",maxWidth:"500px",margin:"0 auto"}}>
        <div style={{display:"flex",flexDirection:"column",gap:"18px"}}>
          <div><label style={labelStyle}>Job Name</label><input type="text" value={newJobName} onChange={e=>setNewJobName(e.target.value.toUpperCase())} placeholder="e.g. SHAKE SHACK" style={{...inputStyle,textTransform:"uppercase"}}/></div>
          <div><label style={labelStyle}>Customer Name</label><input type="text" value={newJobCustomer} onChange={e=>setNewJobCustomer(e.target.value)} placeholder="e.g. Mount Kisco Associates, LLC." style={inputStyle}/></div>
          <div><label style={labelStyle}>Job Address</label><AddressInput value={newJobAddress} onChange={e=>setNewJobAddress(e.target.value)} style={inputStyle}/><div style={{fontSize:"11px",color:t.textMuted,marginTop:"4px"}}>Start typing and select from suggestions</div></div>
          <div style={{display:"flex",gap:"10px",marginTop:"8px"}}><button onClick={()=>{setShowAddJob(false);setEditingActiveJob(null);setNewJobName("");setNewJobAddress("");setNewJobCustomer("");}} style={{...baseBtn,flex:1,background:t.card,border:`1.5px solid ${t.border}`,color:t.textMuted,padding:"14px"}}>Cancel</button><button onClick={saveActiveJob} style={{...primaryBtn,flex:2}}>{editingActiveJob!==null?"Update Job":"Add Job"}</button></div>
        </div>
      </div>
    </div>
  );
  ...
  // ── HOME ──
  if(mode===null)return(
    <div style={{minHeight:"100vh",background:t.bg,fontFamily:"'DM Sans',sans-serif",padding:"24px 16px",display:"flex",flexDirection:"column",alignItems:"center"}}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=Playfair+Display:wght@700&display=swap" rel="stylesheet"/><Toast/>
      <div style={{textAlign:"center",marginBottom:"20px"}}><Logo size={70}/><h1 style={{fontFamily:"'Playfair Display',serif",fontSize:"24px",color:t.text,margin:"12px 0 0"}}>Work Orders</h1><p style={{color:t.textMuted,fontSize:"13px",marginTop:"4px"}}>Create and view daily crew assignments</p></div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",width:"100%",maxWidth:"400px",marginBottom:"12px"}}>
        <button onClick={()=>setPinDialog("manager")} style={{...baseBtn,background:t.card,border:`1.5px solid ${t.border}`,padding:"16px 10px",borderRadius:"14px",flexDirection:"column",gap:"4px",color:t.text,position:"relative"}}>
          <span style={{display:"flex",alignItems:"center",gap:"4px"}}><LockIcon/><span style={{fontSize:"14px",fontWeight:700}}>Manager</span></span><span style={{fontSize:"10px",color:t.textMuted,fontWeight:400}}>Create & manage work orders</span>{managerUpdates&&<Dot/>}
        </button>
        <button onClick={()=>setMode("crew")} style={{...baseBtn,background:t.card,border:`1.5px solid ${t.border}`,padding:"16px 10px",borderRadius:"14px",flexDirection:"column",gap:"4px",color:t.text,position:"relative"}}>
          <span style={{fontSize:"14px",fontWeight:700}}>Crew</span><span style={{fontSize:"10px",color:t.textMuted,fontWeight:400}}>View today's assignments</span>{crewUpdates&&<Dot/>}
        </button>
        <button onClick={()=>setMode("fieldops")} style={{...baseBtn,background:t.card,border:`1.5px solid ${t.border}`,padding:"16px 10px",borderRadius:"14px",flexDirection:"column",gap:"4px",color:t.text,position:"relative"}}>
          <span style={{fontSize:"14px",fontWeight:700}}>Field Operations</span><span style={{fontSize:"10px",color:t.textMuted,fontWeight:400}}>Joe & Bryan work orders</span>{fieldUpdates&&<Dot/>}
        </button>
        <button onClick={()=>setMode("fieldnotes")} style={{...baseBtn,background:t.card,border:`1.5px solid ${t.border}`,padding:"16px 10px",borderRadius:"14px",flexDirection:"column",gap:"4px",color:t.text,position:"relative"}}>
          <span style={{display:"flex",alignItems:"center",gap:"4px"}}><NoteIcon/><span style={{fontSize:"14px",fontWeight:700}}>Field Notes</span></span><span style={{fontSize:"10px",color:t.textMuted,fontWeight:400}}>Crew notes, photos & reports</span>{notesUpdates&&<Dot/>}
        </button>
        <button onClick={()=>setMode("files")} style={{...baseBtn,background:t.card,border:`1.5px solid ${t.border}`,padding:"16px 10px",borderRadius:"14px",flexDirection:"column",gap:"4px",color:t.text,position:"relative"}}>
          <span style={{display:"flex",alignItems:"center",gap:"4px"}}><FolderIcon/><span style={{fontSize:"14px",fontWeight:700}}>Files</span></span><span style={{fontSize:"10px",color:t.textMuted,fontWeight:400}}>All attachments & documents</span>{filesUpdates&&<Dot/>}
        </button>
        <button onClick={()=>setMode("lockbox")} style={{...baseBtn,background:t.card,border:`1.5px solid ${t.border}`,padding:"16px 10px",borderRadius:"14px",flexDirection:"column",gap:"4px",color:t.text,position:"relative"}}>
          <span style={{display:"flex",alignItems:"center",gap:"4px"}}><KeyIcon/><span style={{fontSize:"14px",fontWeight:700}}>Lock Box Codes</span></span><span style={{fontSize:"10px",color:t.textMuted,fontWeight:400}}>Job site access codes</span>{lockboxUpdates&&<Dot/>}
        </button>
      </div>
      ...
      <div style={{width:"100%",maxWidth:"600px",marginTop:"8px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"}}>
          <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:"20px",color:t.text,margin:0}}>Active Jobs</h2>
          <button onClick={()=>setPinDialog("addJob")} style={{...baseBtn,background:t.accent,color:"#fff",padding:"8px 14px",fontSize:"12px",borderRadius:"8px"}}><PlusIcon/> Add Job</button>
        </div>
        {(activeJobs||[]).length===0?<div style={{textAlign:"center",padding:"24px",color:t.textMuted,fontSize:"13px"}}>No active jobs</div>
        :<div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:"13px"}}>
            <thead>
              <tr>
                <th style={{textAlign:"left",padding:"8px 10px",borderBottom:"2px solid #1A1A1A",fontWeight:700,fontSize:"12px",color:t.text}}>Job Name</th>
                <th style={{textAlign:"left",padding:"8px 10px",borderBottom:"2px solid #1A1A1A",fontWeight:700,fontSize:"12px",color:t.text}}>Customer</th>
                <th style={{textAlign:"left",padding:"8px 10px",borderBottom:"2px solid #1A1A1A",fontWeight:700,fontSize:"12px",color:t.text}}>Address</th>
                <th style={{width:"32px",borderBottom:"2px solid #1A1A1A"}}></th>
              </tr>
            </thead>
            <tbody>
              {(activeJobs||[]).map((job,idx)=>(
                <tr key={idx}>
                  <td style={{padding:"10px",borderBottom:`1px solid ${t.border}`,fontWeight:700,color:t.text,fontSize:"13px"}}>{job.name}</td>
                  <td style={{padding:"10px",borderBottom:`1px solid ${t.border}`,color:t.text,fontSize:"13px"}}>{job.customer||"\u2014"}</td>
                  <td style={{padding:"10px",borderBottom:`1px solid ${t.border}`,color:t.text,fontSize:"13px"}}>{job.address}</td>
                  <td style={{padding:"4px",borderBottom:`1px solid ${t.border}`,textAlign:"center"}}>
                    <button onClick={()=>{
                      setPinDialog({type:"editJob",index:idx});
      ...
      {pinDialog==="addJob"&&<PinDialog title="Enter Admin Code" onSuccess={()=>{setPinDialog(null);setNewJobName("");setNewJobAddress("");setNewJobCustomer("");setEditingActiveJob(null);setShowAddJob(true);}} onCancel={()=>setPinDialog(null)}/>}
      {pinDialog?.type==="editJob"&&<PinDialog title="Enter Admin Code" onSuccess={()=>{
        const idx=pinDialog.index;const job=(activeJobs||[])[idx];
        const action=window.prompt("Type 'edit' to edit or 'delete' to remove:");
        if(action?.toLowerCase()==="edit"){setNewJobName(job.name);setNewJobAddress(job.address);setNewJobCustomer(job.customer||"");setEditingActiveJob(idx);setShowAddJob(true);}
