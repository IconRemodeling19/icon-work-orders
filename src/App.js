...
function getFileType(url,name){
  // Extension from name if it actually has one (has a dot), else from URL path
  const extOf=(s)=>{s=(s||"").split("?")[0];const i=s.lastIndexOf(".");return i>=0?s.slice(i+1).toLowerCase():"";};
            ...
            {(formData.jobs||[{...emptyJob}]).map((job,ji)=>{
              ...
              return(
                <div key={ji} style={{background:jobColors2[ji],border:`1.5px solid ${jobBorders[ji]}`,borderRadius:"14px",padding:"16px",display:"flex",flexDirection:"column",gap:"12px"}}>
                  ...
                  <div><label style={labelStyle}>Customer Email</label><input type="email" value={job.customerEmail||""} onChange={e=>updateJob("customerEmail",e.target.value)} placeholder="customer@email.com" style={inputStyle}/></div>
